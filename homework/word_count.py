...
def line_preprocessing(sequence):
    """Line Preprocessing"""
    sequence=[
        (key, value.translate(str.maketrans("","",string.punctuation))) #elimina la puntuación
        for key, value in sequence
              ]
    sequence=[
        (key, value.lower()) #Vuelve toda la letra en minuscula
        for key, value in sequence
    ]
    sequence=[
        (key,value.strip()) #quita los retornos de carro 
        for key, value in sequence
    ]
    return sequence
